...
export function maybeAddComment(
  desc: Partial<
    Pick<SourceDescription, 'leadingComments' | 'trailingComments'>
  >,
  chunks: string[],
  deprecated?: boolean,
  prefix: string = ''
  ...
  if (desc.leadingComments || desc.trailingComments) {
    let content = (desc.leadingComments || desc.trailingComments || '')
...
export function maybePrefixPackage(
...
export class FormattedMethodDescriptor implements MethodDescriptorProto {
  ...
  public static formatName(methodName: string, options: Options) {
    ...
    if (
      options.lowerCaseServiceMethods ||
      options.outputServices.includes(ServiceOption.GRPC)
    ) {
 ...
 * @param optional
 */
export function getPropertyAccessor(
  objectName: string,
  propertyName: string,
...
export function impFile(options: Options, spec: string) {
  return `${spec}${options.importSuffix}`
}
...
export function impProto(
  ...
  if (options.M[protoFile]) {
    return `${prefix}${type}@${options.M[protoFile]}`
  }
  return `${prefix}${type}@./${module}${options.fileSuffix}${options.importSuffix}`
}

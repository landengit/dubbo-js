...
export function visit(
  ...
  const childEnumType = isRootFile
    ? Fields.file.enum_type
  ...
  const childType = isRootFile
    ? Fields.file.message_type
  ...
  messages.forEach((message, index) => {
    ...
    // I.e. FooBar_ZazInner
    const tsFullName =
      tsPrefix + maybeSnakeToCamel(messageName(message), options)

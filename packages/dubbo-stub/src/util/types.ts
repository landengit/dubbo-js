...
export function basicLongWireType(
...
export function defaultValue(ctx: Context, field: FieldDescriptorProto): any {
  ...
  switch (field.type) {
    ...
    case FieldDescriptorProto_Type.TYPE_ENUM:
      ...
      const zerothValue =
        enumProto.value.find((v) => v.number === 0) || enumProto.value[0]
      if (options.stringEnums) {
        const enumType = messageToTypeName(ctx, field.typeName)
        return `${enumType}.${zerothValue.name}`
      } else {
...
export function notDefaultCheck(
  ...
  switch (field.type) {
    ...
    case FieldDescriptorProto_Type.TYPE_FIXED32:
    case FieldDescriptorProto_Type.TYPE_SFIXED32:
      return `${maybeNotUndefinedAnd} ${place} !== 0`
    case FieldDescriptorProto_Type.TYPE_ENUM:
      // proto3 enforces enums starting at 0, however proto2 does not, so we have
      // to probe and see if zero is an allowed value. If it's not, pick the first one.
      // This is probably not great, but it's only used in fromJSON and fromPartial,
      ...
      if (options.stringEnums) {
        const enumType = messageToTypeName(ctx, field.typeName)
        return `${maybeNotUndefinedAnd} ${place} !== ${enumType}.${zerothValue.name}`
      } else {
        return `${maybeNotUndefinedAnd} ${place} !== ${zerothValue.number}`
      }
    case FieldDescriptorProto_Type.TYPE_UINT64:
    case FieldDescriptorProto_Type.TYPE_FIXED64:
    case FieldDescriptorProto_Type.TYPE_INT64:
    ...
    case FieldDescriptorProto_Type.TYPE_SFIXED64:
      if (options.forceLong === LongOption.LONG) {
        return `${maybeNotUndefinedAnd} !${place}.isZero()`
      } else if (options.forceLong === LongOption.STRING) {
        return `${maybeNotUndefinedAnd} ${place} !== "0"`
      } else {
        return `${maybeNotUndefinedAnd} ${place} !== 0`
    ...
    case FieldDescriptorProto_Type.TYPE_BOOL:
      return `${place} === true`
    case FieldDescriptorProto_Type.TYPE_STRING:
      return `${maybeNotUndefinedAnd} ${place} !== ""`
    case FieldDescriptorProto_Type.TYPE_BYTES:
      return `${maybeNotUndefinedAnd} ${place}.length !== 0`
    default:
...
export function isWithinOneOfThatShouldBeUnion(
  options: Options,
  field: FieldDescriptorProto
): boolean {
  return (
...
export function isLongValueType(field: FieldDescriptorProto): boolean {
  return (
    field.typeName === '.google.protobuf.Int64Value' ||
    field.typeName === '.google.protobuf.UInt64Value'
  )
}
...
export function valueTypeName(
  ...
  switch (typeName) {
    ...
    case '.google.protobuf.UInt64Value':
      // return options ? longTypeName(options) : `number`;
      return longTypeName(ctx)
    ...
    case '.google.protobuf.ListValue':
      return ctx.options.useReadonlyTypes ? `ReadonlyArray<any>` : `Array<any>`
    case '.google.protobuf.Value':
      return `any`
    case '.google.protobuf.Struct':
      return ctx.options.useReadonlyTypes
    ...
    case '.google.protobuf.Duration':
      return ctx.options.useJsonWireFormat ? `string` : undefined
...
export function requestType(
  ...
  if (partial) {
    typeName = `${ctx.utils.DeepPartial}<${typeName}>`
  }

  if (methodDesc.clientStreaming) {
    return `${observableType(ctx)}<${typeName}>`
  }
  return typeName
}
...
export function responsePromise(
  ...
  return `Promise<${responseType(ctx, methodDesc)}>`
}

export function responseObservable(
  ctx: Context,
  methodDesc: MethodDescriptorProto
): Code {
  return `${observableType(ctx)}<${responseType(ctx, methodDesc)}>`
}
...
export function detectBatchMethod(
  ...
  if (nameMatches && inputType && outputType) {
    ...
    if (
      hasSingleRepeatedField(inputTypeDesc) &&
      hasSingleRepeatedField(outputTypeDesc)
    ) {
      ...
      const mapType = detectMapType(
        ctx,
        outputTypeDesc,
        outputTypeDesc.field[0]
      )
      if (mapType) {
        outputType = mapType.valueType
      }
      const uniqueIdentifier = `${maybePrefixPackage(
        fileDesc,
        serviceDesc.name
      )}.${methodDesc.name}`
      return {
        ...
        uniqueIdentifier,
        singleMethodName: FormattedMethodDescriptor.formatName(
          singleMethodName,
          ctx.options
        ),
        inputFieldName,
...
function hasSingleRepeatedField(messageDesc: DescriptorProto): boolean {
  return (
    messageDesc.field.length == 1 &&
    messageDesc.field[0].label === FieldDescriptorProto_Label.LABEL_REPEATED
  )
}

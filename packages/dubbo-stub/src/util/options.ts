  ...
  parameter.split(',').forEach((param) => {
    ...
    if (key.charAt(0) === 'M') {
      if (typeof value !== 'string') {
        ...
        if (options.M[mKey]) {
          console.warn(
            `received conflicting M options: '${param}' will override 'M${mKey}=${options.M[mKey]}'`
          )
        }
        if (param.endsWith('.ts')) {
          console.warn(
            `received M option '${param}' ending in '.ts' this is usually a mistake`
          )
        }
...
// todo dong 2022/11/21  any 要处理。
export function getTsPoetOpts(_options: Options): any {
  const imports = ['protobufjs/minimal' + _options.importSuffix]
  return {
    prefix: `/* eslint-disable */`,
    dprintOptions: { preferSingleLine: true, lineWidth: 120 },
    ...(_options.esModuleInterop
      ? { forceDefaultImport: imports }
      : { forceModuleImport: imports })
  }
}
